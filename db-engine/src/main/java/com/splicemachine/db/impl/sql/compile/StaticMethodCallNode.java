...
// from here
    ...
    private void resolveRoutine(FromList fromList,
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            ...
            if (parameterCount != methodParms.length) {
                String err = "- Parameter count is wrong (has " + parameterCount + ", needs " +
                        methodParms.length + ")\n  for ";
            ...
            for (int p = 0; p < parameterCount; p++) {
                ...
                if (needCast) {
                    ...
                    {
                        String err =  "- ERROR for parameter " +  p + ": " +
                                se.getSQLState() + ": " + se.getTextMessage();
                        errorList.append(err);
                        errorList.append( ":\n  for " );
                        routineInfo.toStringParameters(errorList);
                        errorList.append("\n");
        ...
        if(this.routineInfo!=null && this.routineInfo.getLanguage() != null &&
                this.routineInfo.getLanguage().equals("PYTHON"))
        {
            callPython();
        }
    ...
    private ValueNode addCastNode(FromList fromList, SubqueryList subqueryList,
                                  List<AggregateNode> aggregateVector, int paramNum,
                                  ValueNode sqlParamNode, DataTypeDescriptor paramdtd) throws StandardException {
        if (sqlParamNode == null) {

            sqlParamNode = (ValueNode) getNodeFactory().getNode(
                    C_NodeTypes.JAVA_TO_SQL_VALUE_NODE,
                    methodParms[paramNum],
                    getContextManager());
        }
        ...
        methodParms[paramNum] = (JavaValueNode) getNodeFactory().getNode(
                C_NodeTypes.SQL_TO_JAVA_VALUE_NODE,
                castNode,
                getContextManager());

        // this can throw an error
        methodParms[paramNum] = methodParms[paramNum].bindExpression(fromList, subqueryList, aggregateVector);
        return sqlParamNode;
    }
